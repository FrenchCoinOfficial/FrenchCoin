...
#if defined(NDEBUG)
# error "FrenchCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0c9d8036b68d80370f393cdbec9de177fb9c23b0a68db128958ee2a1b56fd0a3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FrenchCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FrenchCoin Signed Message:\n";
...
{
    // FrenchCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FrenchCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FrenchCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1515324624, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "01012017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04495a5ccc8ef35d1e81cccce458d29a21b1ac69ebd58139e0c3807444f1be03e9c2bb1b950156cb532b81c8e1e67b7a6a3243910a88eb6e3f41e2b65d425271ce") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515324624;
        ...
        assert(block.hashMerkleRoot == uint256("0xb499fd5c1e33fd7ecc159ab78fc7ed8d12783a9e3c9d7e3698e03e5aa05e13cb"));
...
unsigned char pchMessageStart[4] = { 0x22, 0x2a, 0x12, 0xb8 }; // FrenchCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FrenchCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FrenchCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FrenchCoinMiner(CWallet *pwallet)
{
    printf("FrenchCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("frenchcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FrenchCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FrenchCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FrenchCoinMiner, pwallet));
}
